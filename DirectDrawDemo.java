...
public class DirectDrawDemo extends JPanel {
    ...
    public DirectDrawDemo(int width, int height, MandelbroutSet m) {
        canvas = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        fillCanvas(Color.BLUE);
        //drawRect(Color.RED, 0, 0, width/2, height/2);
        paint_canvas(width, height, m);
    }

    public void paint_canvas(int width, int height, MandelbroutSet m) {
		// TODO Auto-generated method stub
    	for (int x = 0; x < canvas.getWidth(); x++) {
            for (int y = 0; y < canvas.getHeight(); y++) {
                canvas.setRGB(x, y, m.get_RGBcolor(m.matrix[x][y]));
            }
        }
        repaint();
	}
